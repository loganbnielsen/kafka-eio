...
(** [consumer_queue_events_enable handle write_fd] registers [write_fd] with
    ...
    [consumer_poll handle 0] after waking on the matching read end — plain
    [rd_kafka_consume_queue] reads bypass the consumer-group housekeeping
    [rd_kafka_consumer_poll] performs (heartbeats, rebalance callbacks, and
    resetting the [max.poll.interval.ms] watchdog), so a consumer drained
    that way is silently kicked from its group every [max.poll.interval.ms]
    regardless of actual throughput — [consumer_poll] reads from this same
    queue, so the event-driven wakeup here still applies. *)
